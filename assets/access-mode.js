...
(function(){
  ...
  function toast(msg){
    try {
      ...
      Object.assign(n.style,{position:'fixed',zIndex:'9999',bottom:'16px',left:'50%',transform:'translateX(-50%)',padding:'10px 14px',background:'rgba(0,0,0,.7)',color:'#fff',font:'600 12px/1 Inter, system-ui, sans-serif',borderRadius:'999px',boxShadow:'0 8px 24px rgba(0,0,0,.2)'});
      document.body.appendChild(n); setTimeout(()=>n.remove(),1400);
    } catch(_){}
  }

  function disableInteractive(el){
    // allow if the element OR any ancestor is data-view-allowed
  ...
  function blockEvents(root){
    ...
    ['click','input','change','keydown','keyup','submit','dragstart','drop'].forEach((ev)=>{
      root.addEventListener(ev, function(e){
        // if the target or its ancestors are in an allowed area, let it through
  ...
  function disableAnchors(){
    document.querySelectorAll('a[data-view-block], a[data-lockable]').forEach((a)=>{
      if (a.closest('[data-view-allowed]')) return; // allowed zone
  ...
  function applyViewer(){
    ...
    all.forEach(disableInteractive);
    ...
    // prevent submit unless form itself is allowed
  ...
  // public helpers (called from editor-tray.js)
  async function enterEditorIfAuthed(){
    const sb = window.$sb;
    if (!sb) { setMode('viewer'); applyViewer(); return alert('Auth not ready'); }
    const { data } = await sb.auth.getUser();
    if (data?.user) { setMode('editor'); applyEditor(); toast('Editor mode'); }
    else { alert('Please log in first.'); }
  }
  function exitEditor(){ setMode('viewer'); applyViewer(); toast('Viewer mode'); }

  window.TSN_View = { enterEditorIfAuthed, exitEditor };

  // Init: default viewer; upgrade to editor only if a user session exists
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    setMode('viewer'); applyViewer(); // default
    waitForSB((sb)=>{
